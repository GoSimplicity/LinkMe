...
type Participant struct {
	...
	ActivityID     string // 关联的活动ID（可以是抽奖或秒杀活动）
	UserID         string // 参与者的用户ID
	ParticipatedAt int64  // UNIX 时间戳，表示参与时间
}

// LotteryDraw 表示一个抽奖活动
type LotteryDraw struct {
	ID           string        // 抽奖活动的唯一标识符
	Name         string        // 抽奖活动名称
...
// CreateLotteryDrawInput 是创建抽奖活动的输入参数
type CreateLotteryDrawInput struct {
	Name        string // 抽奖活动名称
	Description string // 抽奖活动描述
	StartTime   int64  // UNIX 时间戳，表示活动开始时间
	EndTime     int64  // UNIX 时间戳，表示活动结束时间
...
// CreateSecondKillEventInput 是创建秒杀活动的输入参数
type CreateSecondKillEventInput struct {
	Name        string // 秒杀活动名称
	Description string // 秒杀活动描述
	StartTime   int64  // UNIX 时间戳，表示活动开始时间
	EndTime     int64  // UNIX 时间戳，表示活动结束时间
