...
type Interactive struct {
	...
	BizID        uint   `gorm:"uniqueIndex"`                                  // 业务ID，用于标识互动的业务对象
...
func (i *interactiveDAO) DeleteLikeInfo(ctx context.Context, lb UserLikeBiz) error {
	...
	return i.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		err := tx.WithContext(ctx).Model(&UserLikeBiz{}).Where("uid = ? AND biz_id = ?", lb.Uid, lb.BizID).Updates(map[string]interface{}{
			"updated_at": now,
		...
		return tx.WithContext(ctx).Model(&Interactive{}).Where("biz_id = ?", lb.BizID).Updates(map[string]interface{}{
			"like_count": gorm.Expr("`like_count` - 1"),
			"updated_at": now,
		}).Error
	})
...
func (i *interactiveDAO) DeleteCollectionBiz(ctx context.Context, cb UserCollectionBiz) error {
	...
	return i.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		// 更新用户收藏信息
		err := tx.WithContext(ctx).Model(&UserCollectionBiz{}).Where("uid = ? AND biz_id = ? AND collection_id = ?", cb.Uid, cb.BizID, cb.CollectionId).Updates(map[string]interface{}{
			"updated_at": now,
		...
		// 更新互动信息
		return tx.WithContext(ctx).Model(&Interactive{}).Where("biz_id = ?", cb.BizID).Updates(map[string]interface{}{
			"collect_count": gorm.Expr("`collect_count` - 1"),
			"updated_at":    now,
		}).Error
	})
...
func (i *interactiveDAO) GetLikeInfo(ctx context.Context, postId uint, uid int64) (UserLikeBiz, error) {
	var lb UserLikeBiz
	err := i.db.WithContext(ctx).Where("uid = ? AND biz_id = ? AND status = ?", uid, postId, 1).First(&lb).Error
	return lb, err
}
...
func (i *interactiveDAO) GetCollectInfo(ctx context.Context, postId uint, uid int64) (UserCollectionBiz, error) {
	...
	err := i.db.WithContext(ctx).Where("uid = ? AND biz_id = ? AND status = ?", uid, postId, 1).First(&cb).Error
	return cb, err
}
